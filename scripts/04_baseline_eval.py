#!/usr/bin/env python3
"""Zero-training baseline: run YOLO11n + BoT-SORT over SoccerNet clips and report
HOTA / MOTA / IDF1 against ground truth — the number every later rung must beat.

Ground truth is normalised to a single pedestrian class (players + ball lumped),
matching the un-classed tracks our pipeline emits.

    python scripts/04_baseline_eval.py --split test --max-seqs 5   # quick subset
    python scripts/04_baseline_eval.py --split test                # full split
"""
import argparse
import json
import sys
...
def load_gt_rows(gt_txt: Path):
    """Parse MOTChallenge gt.txt -> [(frame,id,x,y,w,h)] (first 6 cols only)."""
    rows = []
    for line in Path(gt_txt).read_text().splitlines():
        if not line.strip():
            continue
        p = line.split(",")
...
def main() -> None:
    ...
    ap.add_argument("--splits-file", default="outputs/soccernet_splits.json")
    ...
    ap.add_argument("--out-dir", default="outputs/soccernet_baseline")
    args = ap.parse_args()

    data = json.loads(Path(args.splits_file).read_text())
    seqs = [s for s in data[args.split] if s.get("has_gt")]
    if args.max_seqs:
        seqs = seqs[: args.max_seqs]
    if not seqs:
        sys.exit(f"No GT sequences in split '{args.split}' of {args.splits_file}. "
                 f"Run scripts/03_make_splits.py first.")

    subset = f"  [SUBSET: first {len(seqs)}]" if args.max_seqs else ""
    print(f"Baseline on split='{args.split}'{subset} — {len(seqs)} sequence(s)\n")

    pred_dir = Path(args.out_dir) / "trackers"
    gt_tmp = Path(tempfile.mkdtemp(prefix="pv_sngt_"))
    seq_lengths = {}
    for k, s in enumerate(seqs, 1):
        seq_path, name = Path(s["path"]), s["name"]
        ...
        write_gt(gt_tmp, name, load_gt_rows(seq_path / "gt" / "gt.txt"))
    ...
    m = res[TRACKER]
    print("\n==== BASELINE (YOLO11n + BoT-SORT, zero training) ====")
