...
**Honest caveat — per-class (`scripts/11_perclass_eval.py`, dev):** the win is *people-driven*. The person class (players + goalkeepers + referees) reaches HOTA **0.64**, but the **ball only 0.12** (DetA **0.15** — rarely caught: ~10 px, fast, single-instance, ~6% of the fine-tuning boxes). Fixing ball tracking — higher-res/crop inference, ball oversampling, ball-specific tracker params — is the explicit next target. **Occlusion & identity** (`scripts/12_occlusion_eval.py`, dev): across **1,510** genuine on-screen occlusions — a player overlapped by *another player*, entered **and** exited while interior to the frame — the tracker **keeps the player's identity 91.0%** of the time when it detects them on both sides (endpoint coverage **84.9%**). So association *through* occlusion is strong; the real weakness is **detection continuity** — each player track-instance is still covered by **2.8 tracker IDs** on average (only **26%** survive under a single ID). That reframes Rung 4 around not *losing* the player, rather than re-identifying them afterwards. *(An earlier "≈4% occlusion recovery" figure was **withdrawn**: an adversarial audit showed 25 of its 27 "gaps" were frame-edge exits, and 26 of 27 apparent failures were **detection** rather than identity failures. This benchmark replaces it and reports coverage and retention as separate numbers.)* Provenance: [`results/rung3_baseline.json`](results/rung3_baseline.json).
