...
**+21% HOTA on the sealed match, every metric up, ID switches ~halved** — and dev (0.625) ≈ final (0.596), so the gain generalises across matches rather than overfitting the dev game. The detector was fine-tuned on a **fingerprint-verified `{6,9}`-only export** — *provably* leave-one-game-out (it never saw game 4 or game 2; see [`results/rung3_provenance/`](results/rung3_provenance/)). What moved it: an ablation showed **detector strength dominates** (~3–4× the tracker-tuning gain; 3.4× when matched on the same clips), and fine-tuned yolo11m even beats *zero-shot* yolo11x (0.564) — domain adaptation > raw model size.

**Honest caveat — per-class (`scripts/11_perclass_eval.py`, dev):** the win is *people-driven*. The person class (players + goalkeepers + referees) reaches HOTA **0.64**, but the **ball only 0.12** (DetA **0.15** — rarely caught: ~10 px, fast, single-instance, ~6% of the fine-tuning boxes). Fixing ball tracking — higher-res/crop inference, ball oversampling, ball-specific tracker params — is the explicit next target. **Identity fragmentation** (`scripts/12_occlusion_eval.py`): each GT person *track-instance* (player/GK/referee, per clip) is covered by **2.8 tracker IDs** on average — only **26%** survive under a single ID. *(Correction: an earlier "occlusion recovery ≈4%" figure has been **withdrawn**. An adversarial audit showed 25 of its 27 "visibility gaps" were frame-edge exits rather than occlusions, and 26 of the 27 apparent failures were **detection** failures — where both gap endpoints were actually detected, identity was retained. A genuine on-screen occlusion benchmark — interior, mutual-overlap events with endpoint-coverage reported separately — is being built for Rung 4.)* Provenance: [`results/rung3_baseline.json`](results/rung3_baseline.json).
